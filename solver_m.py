...
class Solver(object):
    ...
    def build_model(self):
        ...
        self.G = Generator(self.g_conv_dim, 8, self.g_repeat_num)
        self.D = Discriminator(self.image_size, self.d_conv_dim, 8, self.d_repeat_num) 
    ...
    def train(self):
        ...
        a = next(data_iter)#edbug
        print(a)   #debug
        #x_fixed, c_org = next(data_iter)  源
        x_fixed, c_org = a  #debug
            ...
            if (i+1) % self.log_step == 0:
                ...
                if self.use_tensorboard:
                    for tag, value in loss.items():
                        ...
                        #self.logger.scalar(tag, value, i+1)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                c_trg_list = self.create_labels(c_org, 8, self.dataset, self.selected_attrs)

                # Translate images.
                x_fake_list = [x_real]
                #print(len(c_trg_list))  8
                #print(len(c_trg_list[0]))  最多16
                for c_trg in c_trg_list:
                    x_fake_list.append(self.G(x_real, c_trg))
                
                #print(type(x_fake_list[5][0])) torch
                for it in x_fake_list[5]:
                    re_img.append(self.denorm(it)) #----------------------------------------------re
                    """-------------------------------------------------------------see
                    ppath = os.path.join('SEE/','{}-see.jpg'.format(count))
                    count+=1
                    save_image(self.denorm(it),ppath)#--------5
                    """
